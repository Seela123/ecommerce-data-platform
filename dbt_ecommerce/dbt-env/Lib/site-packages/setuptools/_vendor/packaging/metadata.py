...
class RawMetadata(TypedDict, total=False):
    """A dictionary of staging core metadata.
...
def parse_email(data: bytes | str) -> tuple[RawMetadata, dict[str, list[str]]]:
    ...
    # We need to cast our `staging` to a metadata, because a TypedDict only support
...
class _Validator(Generic[T]):
    ...
    called with the field's staging value. If the staging value is valid it is returned
    in its "enriched" form (e.g. ``version.Version`` for the ``Version`` field).
    If the staging value is invalid, :exc:`InvalidMetadata` is raised (with a cause
    as appropriate).
    """
